...
def benchmark_qft_circuit(num_qubits: int) -> BenchmarkResult:
    ...
    def qft_circuit():
        ...
        for i in range(num_qubits):
            ...
            for j in range(i + 1, num_qubits):
                angle = np.pi / (2 ** (j - i))
                qml.ctrl(qml.PhaseShift(angle), control=j)(wires=i)
    ...
    num_gates = num_qubits + (num_qubits * (num_qubits - 1)) // 2 + num_qubits // 2
...
def main():
    ...
    print("Starting PennyLane benchmarks...", file=sys.stderr)
    ...
    print(f"PennyLane benchmarks completed in {total_time:.2f}ms", file=sys.stderr)
